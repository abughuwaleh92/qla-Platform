...
app.use(helmet({
  contentSecurityPolicy: {
    directives: {
      ...
      connectSrc: ["'self'", "https://api.vimeo.com", "https://www.youtube.com", "https://accounts.google.com"],
...
// ============================================
app.use(session({
  secret: process.env.SESSION_SECRET || 'qla-lms-secret-key-' + Date.now(),
  resave: false,
  ...
  name: 'qla.sid' // Custom session name
}));
...
const upload = multer({ 
  ...
  fileFilter: (req, file, cb) => {
    ...
    if (mimetype && extname) {
      ...
      cb(new Error('Invalid file type. Allowed types: images, videos, documents'));
    }
  }
});
...
passport.use(new GoogleStrategy({
  ...
  try {
    ...
    // Check if email is from QLA domain (only in production)
    const allowedDomain = process.env.ALLOWED_EMAIL_DOMAIN || '@qla.qfschools.qa';
    if (process.env.NODE_ENV === 'production' && !email.endsWith(allowedDomain)) {
    ...
    if (userResult.rows.length === 0) {
      ...
      let role = 'student';
      let grade = 7; // default grade
      
      if (email.includes('teacher') || email.includes('staff') || email.includes('admin')) {
        role = 'teacher';
        grade = null;
      }
      ...
      await pool.query(
        'UPDATE users SET last_login = NOW(), profile_picture = $2 WHERE id = $1',
        [userResult.rows[0].id, profile.photos && profile.photos[0] ? profile.photos[0].value : userResult.rows[0].profile_picture]
      );
  ...
  res.status(401).send(`
    ...
    <head>
      ...
      <style>
        ...
        h1 { color: #dc2626; }
        a { display: inline-block; margin-top: 20px; padding: 10px 20px; background: #6C1D45; color: white; text-decoration: none; border-radius: 5px; }
      </style>
    </head>
    <body>
      <div class="error-box">
        <h1>Login Failed</h1>
        <p>Only @qla.qfschools.qa email addresses are allowed.</p>
        <a href="/">Try Again</a>
      </div>
    </body>
    </html>
...
app.post('/api/assessments/submit', isStudent, async (req, res) => {
  try {
    ...
    if (questions.length === 0) {
      return res.status(400).json({ error: 'No assessment questions available' });
    }
    ...
    const passPercentage = parseInt(process.env.PASS_PERCENTAGE) || 70;
    ...
    const result = await pool.query(
      ...
      RETURNING *`,
      [req.user.id, lesson_id, score, totalPoints, percentage, passed, time_taken || 0, JSON.stringify(results)]
    );
